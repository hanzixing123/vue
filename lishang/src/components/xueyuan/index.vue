  ...
  <div style="width: 100%">
        ...
        <div class="sosuo">
          <div style="float:right;margin-top:9px;margin-left:5px;">
            <el-select v-model="select" slot="prepend" placeholder="课程" style="width:20%">
              <el-option v-for="(item,indexs) in kc_list" :key="indexs" :label="item.name" :value="item.id"></el-option>
            </el-select>
            <el-input placeholder="请输入内容" v-model="keyword" style="width:70%;margin-left:-5px;">
    ...
    <div class="el-main1">
      <table class="table">
        <tr align="center">
          <td>
            <input type="checkbox" class="kuang" @click="checkAll" :checked="selectList.length == list.length ? y_checked : n_checked"/>
          </td>
          <td>学员名称</td>
          <td>性别</td>
          <td>所选课程</td>
        ...
        <tr v-for="(res, index) in list" :key="index" align="center">
          <td>
            <input type="checkbox" class="kuang" v-model="selectList" :value="res.name + '+' + res.id"/>
          </td>
          <td style="float:left">
            <span class="kuang-1" />
      ...
      <div style="margin-left: 550px">
        ...
        <div v-else>
          <el-pagination class="pagenation" :page-size="pagesize" background layout="prev, pager, next" :total="counts" @current-change="handleCurrentChange"></el-pagination>
        </div>
      </div>
      <!-- 购买课程 -->
      <el-dialog :title="title" :visible.sync="gouke">
        <el-form :model="form2">
          ...
          <el-form-item class="kiuhs" label="* 签约课程" :label-width="formLabelWidth">
            ...
            <el-select style="margin-left: -70px" v-model="form2.courseid" placeholder="请选择">
              <el-option v-for="(item, index) in kc_list" :key="index" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item style="margin-left: 280px; margin-top: -100px; width: 250px" label="课时数" :label-width="formLabelWidth">
            <br />
            <el-input v-model="form2.coursecounts" autocomplete="off" style="margin-left: -80px"></el-input>
          </el-form-item>
          <el-form-item style="margin-left: 420px; margin-top: -100px; width: 250px" label="课程单价" :label-width="formLabelWidth">
            <br />
            <el-input v-model="form2.price" autocomplete="off" style="margin-left: -80px"></el-input>
          </el-form-item>
          <el-form-item style="margin-left: 560px; margin-top: -100px; width: 250px" label="* 课程金额" :label-width="formLabelWidth">
            <br />
          ...
          <el-form-item label="优惠金额" style="width: 300px; margin-left: 200px; margin-top: -100px" :label-width="formLabelWidth">
            <br />
          ...
          <el-form-item label="备注" style="margin-left: -50px" :label-width="formLabelWidth">
            <br />
            <el-input style="margin-left: -50px; width: 250px" type="textarea" :rows="2" placeholder="请输入内容" v-model="form2.remarks"></el-input>
          </el-form-item>
        </el-form>
        ...
        <font color="red" style="font-size:18px;">
          已优惠
          <span style="color: red">￥{{ form2.coursecounts * form2.price - total }}</span>
        </font>
      ...
      <el-dialog :title="titles" :visible.sync="dialogFormVisible">
        <el-form :model="form" class="formssa">
          ...
          <el-form-item label="性别" :label-width="formLabelWidth">
            <el-radio v-model="form.sex" label="1" class="el-radio__input is-checked">男</el-radio>
      ...
      <el-dialog :visible.sync="dialogFormVisible1">
        <ul style="height: 40px">
          <li v-for="(res, index) in selectList" :key="index" style=" font-size: 20px; margin-right: 10px; width: 100px; background: yellow; float: left;text-align: center;" >
            ...
            <el-tab-pane label="插班排课" >
              ...
              <el-table :data="banji_list" stripe style="width: 100%">
                ...
                <el-table-column label="操作">
                  <el-button :plain="true">课表</el-button>
                </el-table-column>
              </el-table>
            </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    xueyuan_add() {
      ...
      let that = this;
       if(that.form.name==""){
            that.addtexta="名字不能为空"
      }else if(that.form.tel==""){
          that.addtexta="手机号不能为空"
      }else if(that.form.sex==""){
          that.addtexta="学员的性别不能为空"
      }else if(that.form.birthday==""){
         that.addtexta="日期不能为空"
      }else if (that.form.num==""){
          that.addtexta="学员的编号不能为空"
      }else if(that.form.remarks==""){
          that.addtexta="备注不能为空"
      }else {
      that.$http.post(
        "/api/students/add",
